        ...
        try {
          ...
          if (imageObj && imageObj.data) {
            ...
            if (trxRefNo && pickwaveMap[trxRefNo]) {
              ...
              // Smart Match: Check if the AI's Last Name exists ANYWHERE in the Pickwave's Full Name string
              for (const pk of Object.keys(pickwaveMap)) {
                if (pk.includes(extractedLastName)) {
                  matchedKey = pk;
                  console.log(`[Page ${i}] Smart Matched AI "${extractedLastName}" to Pickwave "${pk}"`);
                  break;
                }
              }
            }

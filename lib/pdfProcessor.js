...
export function parsePickwaveRecords(fullText) {
  ...
  // IMPORTANT: pdf-parse merges columns with NO space, so "87703-I" is immediately followed by
  // the item name: "87703-IHOWLING BEAGLE...". Do NOT require \s after the SKU.
  const SKU_LINE_RE = /^(\d{5}(?:-[A-Z]|-\d{1,2})?)/i;

  // Order References are exactly 8 digits. Use lookaheads because pdf-parse merges the date and
  // customer name with no spaces: "6/5/2026 1:32:37 PM48832750Pamela Bowen"
  const ORDER_REF_RE = /(?<!\d)(\d{8})(?!\d)/g;
  ...
  for (const line of lines) {
    ...
    // Collect any Order References on this line and map them to the current SKU
    if (currentSku) {
      ORDER_REF_RE.lastIndex = 0;
      let refMatch;
      while ((refMatch = ORDER_REF_RE.exec(trimmed)) !== null) {
        records.push({ orderRef: refMatch[1], sku: currentSku });
      }
    }
  }

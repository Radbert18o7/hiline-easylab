  ...
  try {
    ...
    for (let i = 1; i <= parsedPdf.numPages; i++) {
      ...
      for (const item of textContent.items) {
        ...
        if (/^\d{8}$/.test(text)) {
          orderRefs.push({ ref: text, index: globalIndex });
        } 
        // SKU pattern: 5 digits, optionally followed by dashes and alphanumerics (e.g., 78669-BK, 78669-BK-12)
          ...
          skus.push({ sku: text, index: globalIndex });
        }
    ...
    for (const ref of orderRefs) {
      ...
      if (precedingSkus.length === 0) continue; 
      
      const associatedSku = precedingSkus[precedingSkus.length - 1].sku;
      records.push({ orderRef: ref.ref, sku: associatedSku, used: false });
    }

    console.log(`[parsePickwaveFromBuffer] Found ${skus.length} SKUs, ${orderRefs.length} OrderRefs. Grouped into ${records.length} records.`);
  } catch (err) {
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  let pickwaveList = await parsePickwaveFromBuffer(pickwaveBuffer, pdfjsLib);
  console.log(`[processLabelsPdf] Pickwave loaded: ${pickwaveList.length} records`);
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    const record = pickwaveList.find(r => r.orderRef === trxRefNo && !r.used);
    if (!record) {
      console.log(`[Stamp ${i+1}] SKIP: TrxRef="${trxRefNo}" NOT found in Pickwave (or all SKUs already printed)!`);
      continue;
    }
    
    // Mark this SKU record as used so the next label for the same order gets the next SKU
    record.used = true;

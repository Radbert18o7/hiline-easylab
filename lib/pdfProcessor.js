  ...
  const promises = pngBuffers.map(async (buf, idx) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[CloudflareFallback] Image ${idx} JSON parse failed: ${err.message}. Running fuzzy extraction...`);
        const trxMatch = rawResponse.match(/\b(\d{8})\b/);
        if (trxMatch) parsed.trxRef = trxMatch[1];
        
        if (pickwaveKeys && pickwaveKeys.length > 0) {
          const upperRaw = rawResponse.toUpperCase();
          for (const pk of pickwaveKeys) {
            const words = pk.split(/\s+/);
            for (const word of words) {
              if (word.length >= 3 && !['STORE', 'DEPOT', 'HOME', 'SHIP'].includes(word) && upperRaw.includes(word)) {
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  // Batch process all images via Gemini Flash with pacing to prevent 429s
  const batchSize = 10;
  for (let b = 0; b < pageDataList.length; b += batchSize) {
    ...
    if (unMatchedWithImages.length > 0) {
      if (b > 0) {
        console.log('[processLabelsPdf] Pacing API requests (4s delay)...');
      ...
      for (let j = 0; j < unMatchedWithImages.length; j++) {
        ...
        // Final fallback to text layer matching
        if (!pageObj.matchedKey) {
          const words = pageObj.fullText.toUpperCase().split(/\s+/);
          for (const word of words) {
            const cleanWord = word.replace(/[^A-Z]/g, '');
            if (cleanWord.length > 2 && !['STORE', 'DEPOT', 'HOME', 'SHIP'].includes(cleanWord)) {

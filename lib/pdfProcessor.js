...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    const isOneOfOne = packageSequence && /1\s*(OF|\/)\s*1/i.test(packageSequence);
    const isOneOfOneText = fullText && /1\s*(OF|\/)\s*1/i.test(fullText);

    // Trigger duplication ONLY if the label physically says "1 of 1" but there are multiple SKUs
    if ((isOneOfOne || isOneOfOneText) && skuList.length > 1) {
      // The label mistakenly printed 1 of 1 despite multiple SKUs on pickwave!
      // We must explicitly split/duplicate this label for each SKU
      for (let k = 0; k < skuList.length; k++) {
        ...
        // Add the "X of Y" overlay
        const ofText = `${k + 1} of ${skuList.length}`;
        copiedPage.drawText(ofText, { x: textX, y: textY - 12, size: Math.max(8, fontSize - 1), font, color: rgb(0,0,0) });

        newPdfDoc.addPage(copiedPage);
        console.log(`[Stamp ${i+1}] ✓ Key="${matchedKey}" → ${skuText} (${k + 1} of ${skuList.length})`);
      }
      seenCounts[matchedKey] = (seenCounts[matchedKey] || 0) + skuList.length;
      matchedCount++;
    } else {
      // Original working approach (1-to-1)

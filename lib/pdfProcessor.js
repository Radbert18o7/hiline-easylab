...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    const totalLabelsForThisKey = labelCounts[matchedKey] || 1;
    const idx = seenCounts[matchedKey] || 0;
    seenCounts[matchedKey] = idx + 1;
    
    // Distribute SKUs evenly across the available labels for this key
    const skusPerLabel = Math.ceil(skuList.length / totalLabelsForThisKey);
    const startIndex = idx * skusPerLabel;
    const endIndex = Math.min(startIndex + skusPerLabel, skuList.length);
    
    const assignedSkus = startIndex < skuList.length 
      ? skuList.slice(startIndex, endIndex) 
      : [skuList[skuList.length - 1]]; // fallback to last SKU if exhausted

    const pdfPage = pdfPages[i];
    const { width: pageWidth, height: pageHeight } = pdfPage.getSize();
    const fontSize = Math.max(7, 10 - (assignedSkus.length - 1)); // Shrink font slightly if many SKUs
    ...
    const skuText = `SKU: ${assignedSkus.join(', ')}`;

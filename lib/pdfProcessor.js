  ...
  try {
    ...
    for (let i = 1; i <= parsedPdf.numPages; i++) {
      ...
      for (let j = 0; j < items.length; j++) {
        ...
        // 1. Detect new SKU block (e.g. 78669-BK, 87983S-GY, 79590, 77182-C)
        // SKUs are on the left half, so x < 350. This strictly prevents 5-digit street addresses from being parsed as SKUs.
        if (/^\d{5}[A-Z]?(?:-[A-Z0-9]+)*$/.test(text) && x < 350) {
          currentBlock = { sku: text, skuY: y, totalQty: 1, customers: [] };
          skuBlocks.push(currentBlock);
        }
        
        if (!currentBlock) continue;

        // 2. Extract Quantity for the CURRENT block (must be in the ~379 X-column and on the same Y-axis as currentSku)
        if (/^\d+$/.test(text) && x >= 365 && x <= 395) {
          if (Math.abs(y - currentBlock.skuY) < 5) {
            currentBlock.totalQty = parseInt(text) || 1;
            console.log(`[parsePickwaveFromBuffer] Extracted Quantity=${currentBlock.totalQty} for SKU=${currentBlock.sku}`);
          }
        }

        // 3. Customer Name extraction (after HH:MM:SS PM/AM or HH:MM PM/AM)
        if ((text === 'PM' || text === 'AM') && j > 0 && /^\d{1,2}:\d{2}(:\d{2})?$/.test(items[j-1].str)) {
          ...
          if (nameParts.length > 0) {
            ...
            currentBlock.customers.push(currentName);
          }
        }

        // 4. Order Ref (6 to 10 digits)
        // Must be on the left half of the page to avoid zip codes / street numbers on the far right.
        if (/^\d{6,10}$/.test(text) && x < 350) {
          currentBlock.customers.push(text);
        }
      }
    }

    // 5. Distribute quantities evenly among the customers in each block
    for (const block of skuBlocks) {
      if (block.customers.length === 0) continue;
      const qtyPerCustomer = Math.max(1, Math.floor(block.totalQty / block.customers.length));
      for (const customer of block.customers) {
        if (!pickwaveMap[customer]) pickwaveMap[customer] = [];
        for (let q = 0; q < qtyPerCustomer; q++) {
          pickwaveMap[customer].push(block.sku);
        }
        console.log(`[parsePickwaveFromBuffer] Mapped Name="${customer}" -> ${qtyPerCustomer}x SKU="${block.sku}"`);
      }
    }

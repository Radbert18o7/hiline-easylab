...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  for (const pageObj of pageDataList) {
    ...
    pageResults.push({ matchedKey, packageIndex, billingX, billingY, fullText });
    if (matchedKey) {
  ...
  for (let i = 0; i < numPages; i++) {
    const { matchedKey, billingX, billingY, fullText } = pageResults[i];
    ...
    // Trigger duplication ONLY if the system gave us exactly 1 label but there are multiple SKUs
    if (labelsWeHave === 1 && skuList.length > 1) {

      ...
      for (let j = 0; j < items.length; j++) {
        ...
        if ((text === 'PM' || text === 'AM') && j > 0 && /^\d{1,2}:\d{2}(:\d{2})?$/.test(items[j-1].str)) {
          ...
          while (k < items.length && !/^\d/.test(items[k].str)) {
            // Keep letters and hyphens (to support hyphenated last names)
        ...
        if (/^\d{5}[A-Z]?(?:-[A-Z0-9]+)*$/.test(text)) {
          ...
          // We must check >= 0 to prevent falsely rejecting SKUs on the NEXT page (where Y resets to ~800, making the diff negative).
          if (currentNameY !== null && (currentNameY - y) >= 0 && (currentNameY - y) < 20) {
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    // Add-on approach (Dynamic Allocation / Duplication) is commented out for now.
    /*
    ...
    */
    ...
    // First approach: 1-to-1 mapping
    const idx = seenCounts[matchedKey] || 0;
    seenCounts[matchedKey] = idx + 1;
    const sku = skuList[Math.min(idx, skuList.length - 1)];
    const skusToStampOnThisLabel = [sku];

    for (const skuToStamp of skusToStampOnThisLabel) {
      const [copiedPage] = await newPdfDoc.copyPages(pdfDoc, [i]);

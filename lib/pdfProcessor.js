...
export function parsePickwaveRecords(fullText) {
  ...
  const skuPattern = /(?<!\d)(\d{5}(?:\s*-\s*[A-Z]|\s*-\s*\d{1,2})?)(?!\d)/gi;
  const skus = [];
  let m;
  ...
  while ((m = skuPattern.exec(fullText)) !== null) {
    ...
    // STRICTLY ignore Order Refs (8+ digits), Barcodes/Purchase Nos (6 digits), and known non-SKUs
    // If it's a pure number (no dash), it must be exactly 5 digits and start with 7 or 8 to be a valid Hiline SKU.
  ...
  for (let i = 0; i < orderRefs.length; i++) {
    ...
    // Find the SKU that is closest to this Order Reference (and appears before it)

      ...
      for (let j = 0; j < items.length; j++) {
        ...
        if ((text === 'PM' || text === 'AM') && j > 0 && /^\d{1,2}:\d{2}(:\d{2})?$/.test(items[j-1].str)) {
          ...
          if (nameParts.length > 0) {
            ...
            if (currentSku && !pickwaveMap[currentName].includes(currentSku)) {
              pickwaveMap[currentName].push(currentSku);
              console.log(`[parsePickwaveFromBuffer] Mapped Name="${currentName}" -> SKU="${currentSku}"`);
            }
          }
        }
        ...
        if (/^\d{6,10}$/.test(text) && x < 350) {
          ...
          if (currentSku && !pickwaveMap[currentOrderRef].includes(currentSku)) {

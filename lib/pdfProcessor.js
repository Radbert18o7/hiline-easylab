...
export async function parsePickwaveFromBuffer(buffer, pdfjsLib) {
  ...
  try {
    ...
    let currentNameY = null;
    let currentOrderRef = null;

    for (let i = 1; i <= parsedPdf.numPages; i++) {
      const page = await parsedPdf.getPage(i);
      const textContent = await page.getTextContent();
      const items = textContent.items.map(item => ({ str: item.str.trim(), y: item.transform[5] })).filter(item => item.str);
      
      for (let j = 0; j < items.length; j++) {
        const text = items[j].str;
        const y = items[j].y;

        // 1. Customer Name extraction (after HH:MM:SS PM/AM)
        // This signifies the start of a new order block
        if ((text === 'PM' || text === 'AM') && j > 0 && /^\d{1,2}:\d{2}:\d{2}$/.test(items[j-1].str)) {
          ...
          if (nameParts.length > 0) {
            ...
            currentNameY = y; // Record the Y coordinate of the name block
            console.log(`[parsePickwaveFromBuffer] Started block for Name="${currentName}" at Y=${currentNameY}`);
          }
        }

        // 2. Exact 8 digits for Order Ref
        if (/^\d{8}$/.test(text)) {
          currentOrderRef = text;
        }

        // 3. SKU pattern (e.g. 78669-BK, 87983S-GY, 79590)
        if (/^\d{5}[A-Z]?(?:-[A-Z0-9]+)*$/.test(text)) {
          // If we have a currentName, ensure this SKU is physically located below the Address block.
          // The PDF Y-coordinate decreases down the page.
          // The Address is typically ~10 points below the Name. The Items table is ~30+ points below.
          if (currentNameY !== null && (currentNameY - y) < 20) {
            // It's too close to the Name block! It's likely a street number or ZIP code (e.g. 29687).
            continue;
          }

          if (currentName) {
            if (!pickwaveMap[currentName]) pickwaveMap[currentName] = [];
            if (!pickwaveMap[currentName].includes(text)) {
              pickwaveMap[currentName].push(text);
              console.log(`[parsePickwaveFromBuffer] Mapped Name="${currentName}" -> SKU="${text}"`);
            }
          }
          if (currentOrderRef) {
            if (!pickwaveMap[currentOrderRef]) pickwaveMap[currentOrderRef] = [];
            if (!pickwaveMap[currentOrderRef].includes(text)) {
              pickwaveMap[currentOrderRef].push(text);
            }
          }
        }
      }
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveBuffer) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    // Is this the very last physical label we are going to process for this customer?
    const isLastLabel = seenCounts[matchedKey] === labelsWeHave;

    // Determine which SKUs to stamp on this label
    const skusToStampOnThisLabel = [];
    if (isLastLabel) {
      // Exhaust all remaining SKUs by stamping/duplicating them onto this last label
      for (let k = idx; k < skuList.length; k++) {
        skusToStampOnThisLabel.push(skuList[k]);
      }
      // Fallback if there were fewer SKUs than labels
      if (skusToStampOnThisLabel.length === 0) {
        skusToStampOnThisLabel.push(skuList[Math.min(idx, skuList.length - 1)]);
      }
    } else {
      // Just stamp the 1-to-1 matching SKU for this label
      skusToStampOnThisLabel.push(skuList[Math.min(idx, skuList.length - 1)]);
    }

    for (const sku of skusToStampOnThisLabel) {
      const [copiedPage] = await newPdfDoc.copyPages(pdfDoc, [i]);
      ...
      // If we are duplicating due to Donald's case (multiple SKUs on last label), we can optionally add "X of Y".
      // But standard 1-to-1 stamping doesn't need it. We only add it if skusToStampOnThisLabel.length > 1.
      if (skusToStampOnThisLabel.length > 1) {
        const currentPrintIdx = skusToStampOnThisLabel.indexOf(sku);
        const ofText = `${idx + currentPrintIdx + 1} of ${skuList.length}`;
        copiedPage.drawText(ofText, { x: textX, y: textY - 12, size: Math.max(8, fontSize - 1), font, color: rgb(0,0,0) });
        console.log(`[Stamp ${i+1}] ✓ Key="${matchedKey}" → ${skuText} (${ofText})`);
      } else {
  ...
  return { buffer: Buffer.from(bytes), matchedCount, totalPages: newPdfDoc.getPageCount(), originalPages: numPages };
}

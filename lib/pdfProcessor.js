...
export function parsePickwaveRecords(fullText) {
  ...
  // 3. Match SKUs to the NEXT Order Reference that appears in the document
  let skuIdx = 0;
  for (const ref of orderRefs) {
    const currentOrderSkus = [];
    
    // Consume all SKUs that appear before this orderRef
    while (skuIdx < skus.length && skus[skuIdx].index < ref.index) {
      currentOrderSkus.push(skus[skuIdx].sku);
      skuIdx++;
    }

    if (currentOrderSkus.length > 0) {
      // Create a display string for this order, e.g. "87703-I, 87728-A"
      // If there are duplicates, we count them (e.g. "87703-I (Qty: 2)")
      const counts = {};
      for (const s of currentOrderSkus) {
        counts[s] = (counts[s] || 0) + 1;
      }
      
      const displayParts = [];
      for (const [s, count] of Object.entries(counts)) {
        if (count > 1) displayParts.push(`${s} (Qty: ${count})`);
        else displayParts.push(s);
      }
      
      records.push({ orderRef: ref.ref, sku: displayParts.join(' + ') });
    }
  }
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveRecords) {
  ...
  try {
    ...
    worker = await createWorker('eng', 1, {
      logger: () => {},
      cachePath: os.tmpdir(), // writable on Vercel
      // Let Tesseract download from its official CDN to avoid Vercel missing-file issues
    });

...
export function parsePickwaveRecords(fullText) {
  ...
  // The Pickwave table structure is:
  //   [SKU row]   → starts with: 87703-I  ITEM DESCRIPTION  Qty  Bin  ...
  //   [order row] → contains:    THE HOME DEPOT  date  ORDER_REF  Customer Name  Address
  //
  // Strategy: go line-by-line. When we see a line that starts with a Hiline SKU,
  // set it as the current SKU. Every 8-digit Order Reference we find after that
  // belongs to this SKU — until we hit the next SKU row.

  const lines = fullText.split(/\r?\n/);

  // A valid Hiline SKU starts with 5 digits (beginning with 7 or 8), optionally followed by -letter or -2digits
  // The SKU row format is: SKU  ITEM DESCRIPTION  QUANTITY  BIN(e.g. 3.L.02)  SOURCE
  // We capture the quantity by looking for a small number right before the Bin pattern.
  const SKU_LINE_RE = /^(\d{5}(?:-[A-Z]|-\d{1,2})?)\s/i;
  const QTY_FROM_SKU_LINE_RE = /\s+(\d{1,3})\s+\d+\.[A-Z]+\.\d+/i;

  // Order References are exactly 8 digits
  const ORDER_REF_RE = /\b(\d{8})\b/g;
  ...
  for (const line of lines) {
    ...
    // Check if this line is a SKU header row
    const skuMatch = trimmed.match(SKU_LINE_RE);
    if (skuMatch) {
      const candidate = skuMatch[1].toUpperCase();
      // Must start with 7 or 8 if it is a pure number (e.g. 87938 is valid, 24230 zip code is not)
      if (/^\d+$/.test(candidate) && !/^[78]/.test(candidate)) {
        // not a real SKU header, fall through
      } else {
        currentSku = candidate;
        // Extract quantity: the number right before the Bin (e.g. "3" in "... 3  3.L.02")
        const qtyMatch = trimmed.match(QTY_FROM_SKU_LINE_RE);
        currentQty = qtyMatch ? qtyMatch[1] : null;
        continue;
      }
    }

    // Collect any Order References on this line and map them to the current SKU + quantity
    if (currentSku) {
      ORDER_REF_RE.lastIndex = 0;
      let refMatch;
      while ((refMatch = ORDER_REF_RE.exec(trimmed)) !== null) {
        records.push({ orderRef: refMatch[1], sku: currentSku, qty: currentQty });
      }
    }
  }
...
export async function processLabelsPdf(labelsPdfBuffer, pickwaveRecords) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    const qtyPart = matchedRecord.qty ? `  QTY: ${matchedRecord.qty}` : '';
    const skuText = `SKU ID: "${sku}"${qtyPart}`;
